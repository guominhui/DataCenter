...
<link href="css/mobile/font-awesome.min.css?t=20170928" rel="stylesheet"
	type="text/css" />
</head>
<!-- topbar starts -->
<body>
	...
	<div class="navbar navbar-default" role="navigation">
		<div class="navbar-inner">
			...
			<div class="pull-right"
				...
				<c:if test="${sessionScope.userType==2&&sessionScope.isCloseThreeLevelInstitution!=1}"> 代金券：${sessionScope.bonus}.00 <i
						class="icon-money"></i>
				</c:if>
			</div>
			<!-- user dropdown ends -->
								...
								<table class="table table-bordered table-striped responsive">
									<tbody>
										...
										<form action="sellCardHandle.do" class="testform"
											...
											<input type="hidden" name="toType" value="0"></input> <input
												type="hidden" name="buyerName" value="${username}"></input>
											<tr>
												<td>售卖类型：</td>
												<td><input type="radio" class="sellType"
													name="sellType" value="0" checked />售卖 <input type="radio"
													class="sellType" name="sellType" value="1" />赠送</td>
											</tr>
											<tr id="purchangeClassInfo">
												<td>售卖档位：</td>
												<td><select id="purchangeClass" name="purchangeClass">
														<option value="0">散卖 </option>
														<option value="1">${class1Card}张房卡,花费${class1} </option>
														<option value="2">${class2Card}张房卡,花费${class2} </option>
														<option value="3">${class3Card}张房卡,花费${class3} </option>
												</select></td>
											</tr>

											<tr id="addCardCountInfo">
												<td>增加房卡数：</td>
												<td><input type="number" id="addCardCount"
													name="addCardCount" value="0"
													style="border:1px solid #b4b4b4;width:180px" /></td>
											</tr>
											<tr id="addCardCostInfo">
												<td>入账金额：</td>
												<td><input type="number" id="income" name="income"
													value="0" style="border:1px solid #b4b4b4;width:180px" /></td>
											</tr>
											<tr>
												<td>备注：</td>
	...
	<script language="javascript">
		...
		function sumbit_sure() {
		 ...
		 $(function () {
			$('#purchangeClass').on('change',function(){
			if($(this).val()!=0){
			$('#addCardCountInfo').hide();
			$('#addCardCostInfo').hide();
            $('#addCardCount').val(0);
            
			}else{
			$('#addCardCountInfo').show();
			$('#addCardCostInfo').show();
			}
			});
			
			$('.sellType').on('change',function(){
			if($(this).val()==0){
			$('#purchangeClassInfo').show();
			/* $('#addCardCountInfo').show();
			$('#addCardCostInfo').show(); */
			    
			   if($('#purchangeClass').val()!=0){
			    $('#addCardCountInfo').hide();
			    $('#addCardCostInfo').hide();
			    $('#addCardCount').val(0);
