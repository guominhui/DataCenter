...
public class ConfigUtil {
	...
	public static String[] game_record_open_list;
...
//		} 
	...
	public void handleBonusClassArray(){
		...
		//游戏回访记录查询权限列表
		String gameRecordListStr=(String) ConfigUtil.prop.get("game_record_open_list");
		String[] gameRecordList=gameRecordListStr.split(",");
		if(ee.length<=0){
			System.err.println("请检查三级分销比率");
		}
		game_record_open_list=new String[gameRecordList.length];
		for(int i=0;i<gameRecordList.length;i++){
			game_record_open_list[i]=gameRecordList[i];
		}
