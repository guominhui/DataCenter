...
<body>
	...
	<div class="navbar navbar-default" role="navigation">
		<div class="navbar-inner">
			...
			<div class="pull-right"
				...
				<c:if
					test="${sessionScope.userType==2&&sessionScope.isCloseThreeLevelInstitution!=1}"> 代金券：${sessionScope.bonus}.00 <i
						class="icon-money"></i>
				</c:if>
			</div>
			<!-- user dropdown ends -->
													...
													<c:otherwise>
														<c:forEach items="${list}" varStatus="i" var="item">
															<tr>
																...
																<td class="center "><a class="btn btn-primary" href="#"
																	onclick="addCardCount(${item.userId})"> <i
																		class="glyphicon glyphicon-cog icon-white"></i>
																		加卡
																</a> 
																
																	<a class="btn btn-success" href="#"
																	onclick="editInfo(${item.userId})"> <i
																		class="glyphicon glyphicon-edit icon-white"></i> 修改
																</a> 
																<c:if test="${sessionScope.userType==0}">
																		<a class="btn btn-info" href="#"
																			onclick="clearCardCountOrRemove(${item.userId},0)">
																...
																<c:if test="${sessionScope.userType==0}">
																		...
																		<a class="btn btn-danger" href="#"
																			onclick="clearCardCountOrRemove(${item.userId},1)">
																			<i class="glyphicon glyphicon-trash icon-white"></i>
																			删除
																		</a>
																	</c:if> <%-- <td><input type="button"
	...
	document.getElementById("userId").value= "${userId}";
		...
		function clearCardCountOrRemove(id,ope) {
	         if (!confirm("确定要进行此操作")){
	        return
	         }
			// 创建一个 form 
			...
			form1.method = "POST";
			// form 提交路径 
			form1.action = "clearCardCountOrRemove.do";
			// 对该 form 执行提交 
			form1.submit();
			// 删除该 form 
			document.body.removeChild(form1);
